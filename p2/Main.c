...
-----------------------------  ProducerConsumer  ---------------------------------
  ...
  var
    ...
    bufferMutex: Mutex = new Mutex
    bufferCapacitySemaphore: Semaphore = new Semaphore
    bufferEmptinessSemaphore: Semaphore = new Semaphore


  function ProducerConsumer ()
      bufferMutex.Init()
      bufferCapacitySemaphore.Init(BUFFER_SIZE)
      bufferEmptinessSemaphore.Init(0)
  ...
  function Producer (myId: int)
      ...
      for i = 1 to 5
        ...
        -- Ask semaphore whether there is space to produce
        bufferCapacitySemaphore.Down()

        -- Perform synchronization...
        -- note that it's important to lock mutex after semaphore because it may lock mutex while there is no capacity
        bufferMutex.Lock()
        ...
        -- Signal the waiting consumers for a produced value
        bufferEmptinessSemaphore.Up()
      endFor
    endFunction
  ...
  function Consumer (myId: int)
      var
        c: char
      while true
        -- Ask semaphore whether there is something to consume
        bufferEmptinessSemaphore.Down()
        ...
        -- Signal the waiting producers for free space
        bufferCapacitySemaphore.Up()
      endWhile
    endFunction
  ...
  function PrintBuffer (c: char)
    ...
    --
      ...
      -- Lock the bufferMutex to synchronize operations on buffer through the print process
      -- It can be uncommented if it is assumed to be out of the synchronized area of the produce or consume methods
      -- or the mutex ignore same thread locking in its logic
      -- bufferMutex.Lock()
      ...
      -- Unlock the bufferMutex to allow other thread for using buffer
      -- It can be uncommented in case its Lock pair is uncommented as well
      -- bufferMutex.Unlock()
