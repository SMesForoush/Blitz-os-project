    ...
    --         on the mutex.
      ...
      method Lock ()
          ...
          if self.IsHeldByCurrentThread()
            -- Depend on the convention we can ignore a mutex lock from the same thread as well
            -- In case the convention is to ignore this case comment the first line and uncomment the second line
            FatalError ("Thread locked the owned mutex again.")
            -- return
          endIf

          if isLocked == true
          -- if mutex is locked put current thread to sleep
            waitingThreads.AddToEnd (currentThread)
            currentThread.Sleep ()
          endIf
      ...
      method Unlock ()
          ...
          oldIntStat = SetInterruptsTo (DISABLED)
          if isLocked == false
            -- Depend on the convention we can ignore a mutex unlock when it is already unlocked
            -- In case the convention is to ignore this case comment the first line and uncomment the second line
            FatalError ("The operations tried to unlock an unlocked mutex.")
            -- return
          endIf

          -- check if current thread holds the lock
          if self.IsHeldByCurrentThread() == false
            FatalError ("Thread tried to unlock mutex while it is not the owner")
          endIf

          isLocked = false
          ownerThread = null

          if waitingThreads.IsEmpty() == false
          -- wake up thread waiting longest
            t = waitingThreads.Remove ()
            ownerThread = t
            isLocked = true
            t.status = READY
...
-----------------------------  Condition  ---------------------------------
  ...
  behavior Condition
    ...
    -- on the condition variables to make sure certain conditions are met.
    --
    ...
    -- and execution of the corresponding waiting thread, other threads may
